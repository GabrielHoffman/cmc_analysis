...
### RNA-seq QC metrics
```{r QC, fig.width=18, fig.height=60}
...
ggplot(df, aes( x=CellType, y=value, fill=CellType)) + geom_boxplot() + facet_wrap(~variable, scales="free_y", ncol=5) + theme_bw(10) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5), legend.position="none") + scale_fill_manual("Cell Type", values=colorCodes) 
```
...
df = data.frame(  Name  = colnames(geneExpr), 
                  XIST  = geneExpr[XIST,],
                  UTY   = geneExpr[UTY,])
df = merge( df, info[,c('Name', 'Gender')], by="Name" )
...
```{r sex.off}
kable(df[df$Gender == 'Female' & df$UTY > 5,], digits=2) %>% kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
...
df_RefPanel = lapply( colnames(fitCT), function(coef){
  ...
  df = data.frame(tabl[,'logFC'])
  rownames(df) = rownames(tabl)
  colnames(df) = gsub('CellType', '', coef)
  df

  })
df_RefPanel = do.call(cbind, df_RefPanel)

file = '/hpc/users/hoffmg01/work/cmc_analysis/src/Rousos_MSSM_MolProf_FANS_4_log2CPM_reference_panel.csv.gz'
gz1 <- gzfile(file, "w")
write.table(df_RefPanel, file, quote=FALSE, sep=",")
close(gz1)
```
