...
ggplot(df, aes(CellType, norm.factors, fill=CellType)) + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + scale_color_manual("Cell Type", values=colorCodes) + ylab("TMM factor")
```
...
gz1 <- gzfile(file, "w")
write.table(df_RefPanel[,-exclude], file, quote=FALSE, sep=",")
close(gz1)
