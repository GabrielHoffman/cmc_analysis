...
# read count
gtex_count <- fread(file_counts, header = TRUE, skip = 2, select = c('Description', unique(gtex_sample$SAMPID)))
...
ggplot(df_voomTrend_GTEx, aes(x, y, color=key)) + geom_line() + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + ylab("sqrt( standard deviation)") + xlab("log2( count size + 0.5 )") + ggtitle("GTEx brain regions") + xlim(2, 22) + ylim(0.45, 1.36)
```

```{r gtex.colors}
# https://genome.ucsc.edu/cgi-bin/hgTables?db=hg19&hgta_track=hgFixed&hgta_group=allTables&hgta_table=hgFixed.gtexTissue

gtexColors = read.table('/sc/arion/projects/CommonMind/data/resources/GTEx/TissueColors.tsv')
...
# BCV for FANS4 data
df_cv_FANS = lapply(levels(info$CellType), function(CT){
   
  id = info$Name[which(info$CellType == CT)]

  isexpr = rowSums(cpm(geneCounts[,id])>1) >= 0.3*ncol(geneCounts[,id])

  dge = DGEList(geneCounts[isexpr,id])
  dge = calcNormFactors(dge)
  d = estimateCommonDisp(dge)
  d1 = estimateTrendedDisp(d)
  d2 = estimateTagwiseDisp(d1)

  data.frame( key = CT, Dataset = "FANS4", EnsID = rownames(dge), BCV = sqrt(getDispersion(d2)) )
})
df_cv_FANS = do.call(rbind, df_cv_FANS)

# ggplot( df_cv_FANS, aes(log2(BCV), color=key)) + geom_density() + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + scale_color_manual("Cell Type", values=colorCodes) 


# BCV for GTEX
df_cv_GTEx = lapply( unique(gtex_sample$SMTSD)[c(36)], function(Tissue){

  id = gtex_sample$SAMPID[which( gtex_sample$SMTSD == Tissue)][1:28]

  isexpr = rowSums(cpm(gtex_count[,id])>1) >= 0.3*ncol(gtex_count[,id])

  dge = DGEList(gtex_count[isexpr,id])
  dge = calcNormFactors(dge)
  d = estimateCommonDisp(dge)
  d1 = estimateTrendedDisp(d)
  d2 = estimateTagwiseDisp(d1)

  data.frame( key = Tissue, Dataset = "GTEx", EnsID = rownames(dge), BCV = sqrt(getDispersion(d2)), stringsAsFactors=FALSE )
})
df_cv_GTEx = do.call(rbind, df_cv_GTEx)

ggplot( df_cv_GTEx, aes(log2(BCV), color=key)) + geom_density() + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) 




ggplot( rbind(df_cv_GTEx, df_cv_FANS), aes(log2(BCV), color=key)) + geom_density() + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + facet_wrap(~Dataset)
