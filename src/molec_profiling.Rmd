...
df_RefPanel = lapply( colnames(fitCT), function(coef){
  ...
  df = data.frame(EnsID = rownames(tabl), tabl[,'AveExpr'])
  colnames(df)[2] = gsub('CellType', '', coef)
  df

  })
df_RefPanel = do.call(cbind, df_RefPanel)

file = '/hpc/users/hoffmg01/work/cmc_analysis/src/MolProf_FANS_4_reference_panel.csv.gz'
gz1 <- gzfile(file, "w")
