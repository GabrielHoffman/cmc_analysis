...
```{r GTEX.voom.plot}
ggplot(df_voomTrend_GTEx, aes(x, y, color=key)) + geom_line() + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5), legend.position="below") + ylab("sqrt( standard deviation)") + xlab("log2( count size + 0.5 )") + ggtitle("GTEx brain regions") + xlim(2, 22) + ylim(0.45, 1.36)
```
...
```{r plot.coef.variation, fig.width=12, fig.height=12}
# use colors here
...
ggplot( df, aes(log2(BCV), color=key)) + geom_density() + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5), legend.position="below") + facet_wrap(~Dataset, ncol=1) + scale_color_manual("Type", values=cols) 
```
