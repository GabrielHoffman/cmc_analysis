...
df = melt(info[,c('DissectionID', 'CellType', colnames(qc_metrics)[!exclude][1:3])], id.vars=c('DissectionID', 'CellType'))

ggplot(df, aes( x=CellType, y=value, fill=CellType)) + geom_boxplot() + facet_wrap(~variable, scales="free_y", ncol=6) + theme_bw(10) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + scale_fill_manual("Cell Type", values=colorCodes) 
