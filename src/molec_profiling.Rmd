...
df = melt(info[,c('DissectionID', 'CellType', colnames(qc_metrics)[1:10])], id.vars=c('DissectionID', 'CellType'))



ggplot(df, aes( y=value)) + geom_boxplot() + facet_wrap(~variable) + theme_bw(15) + theme(aspect.ratio=1, plot.title = element_text(hjust = 0.5)) + scale_color_manual("Cell Type", values=colorCodes) 
...
```{r summary}
table(info$Dx)
 length(table(info$Individual.ID))
```
...
```{r dream}
form = ~ (1|CellType) + (1|Individual.ID) + Gender
form = ~ CellType
vobj = voomWithDreamWeights(dge, form, info, plot=TRUE, save.plot=TRUE, BPPARAM=SnowParam(6, progressbar=TRUE))

fitBasic = dream( vobj, ~ CellType + (1|Individual.ID) + Gender, info, BPPARAM=SnowParam(6, progressbar=TRUE))

fitDisease = dream( vobj, ~ CellType*Dx + (1|Individual.ID) + Gender, info, BPPARAM=SnowParam(6, progressbar=TRUE))
```
...
form = ~ CellType
vp = fitExtractVarPartModel( vobj, form, info, BPPARAM=SnowParam(6, progressbar=TRUE))
